...
const PerformanceOverview: React.FC<PerformanceOverviewProps> = ({ mentees }) => {
    try {
        const calculateStats = () => {
            ...
            const cgpaValues = mentees
                .map(s => (s.IA1 + s.IA2 + s.EndSem) / 3)
                .filter(cgpa => cgpa > 0);
            const averageCGPA = cgpaValues.length > 0
                ? Number((cgpaValues.reduce((a, b) => a + b, 0) / cgpaValues.length).toFixed(2))
                : 0;
            ...
            return { totalMentees, averageCGPA, excellentPerformers, studentsWithBacklogs };
        };
        ...
        return (
            <div className="bg-red-50 dark:bg-red-950/30 border border-red-200 dark:border-red-800 rounded-2xl p-6">
                <div className="text-center">
                    <p className="text-red-600 dark:text-red-400 font-semibold">Performance Overview Error</p>
                    <p className="text-sm text-red-500 dark:text-red-400 mt-1">Component: PerformanceOverview | Check console for details</p>
                </div>
            </div>
        );
    }
