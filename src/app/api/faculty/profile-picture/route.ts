...
export async function GET(request: NextRequest) {
    try {
        ...
        const bucketId = process.env.NEXT_PUBLIC_APPWRITE_FACULTY_BUCKET_ID;

        if (!bucketId) {
            return NextResponse.json({ error: 'Faculty storage bucket not configured' }, { status: 500 });
        }
        ...
        if (error.code === 404 || error.type === 'storage_file_not_found') {
            return NextResponse.json({
                error: 'Faculty profile picture not found',
                code: 'FILE_NOT_FOUND'
            }, { status: 404 });
        }
        ...
        // Handle other errors
        return NextResponse.json({
            error: 'Failed to fetch faculty profile picture',
            code: 'FETCH_ERROR'
        }, { status: 500 });
    }
}